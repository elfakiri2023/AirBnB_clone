...
class HBNBCommand(cmd.Cmd):
    ...
    cmd_classes = ['BaseModel', 'Place', 'Review', 'City', 'User', 'Amenity', 'State']
    ...
    def precmd(self, command):
        ...
        if all(char in command for char in ['.', '(', ')']):
            ...
            if method_and_args[0] in HBNBCommand.cmd_actions and class_and_method[0] in HBNBCommand.cmd_classes:
                command = method_and_args[0] + ' ' + class_and_method[0] + ' ' + arguments[0]
    ...
    def do_show(self, arg):
        ...
        if arguments[0] not in HBNBCommand.cmd_classes:
            ...
            for key, value in storage.all().items():
                if value.__class__.__name__ == arguments[0] and value.id == arguments[1].strip('"'):
                    print(value)
                    return
    ...
    def do_destroy(self, arg):
        ...
        if args[0] not in HBNBCommand.cmd_classes:
            ...
            for key, value in storage.all().items():
                if value.id == args[1].strip('"') and value.__class__.__name__ == args[0]:
                    del value
    ...
    def do_update(self, arg):
        ...
        if args[0] not in HBNBCommand.cmd_classes:
            ...
            for key, obj in storage.all().items():
                if obj.id == args[1].strip('"') and obj.__class__.__name__ == args[0]:
                    if args_len == 2:
